...
const STATIC_TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  wiki_context:
    "PRIMARY tool for natural-language wiki questions. Returns Markdown text: '# wiki_context', then '[hit] <path> score=<n.nn> truncated=<bool> [via=<parent>]' followed by the (frontmatter-stripped) page body, ending with a '[cite] …' line. `truncated: true` means the body was clipped to fit `budget_tokens`; call wiki_fetch for the full page. Schema, per-domain indexes, and the recent log are NOT in this output — read them via the `wiki://schema`, `wiki://index/all`, `wiki://log/recent` resources. Wikilink expansion is opt-in via `expand_links: true` (default false). Cite hits with [[path]]. Domain filter accepts a domain name or 'all' (default).",
  wiki_search:
    "Keyword search with two-stage rank: path-token shortlist + body+frontmatter re-rank (title, aliases, tags, entities, concepts, headings). Returns terse JSON: an array of {p,t,s,sn?} where p=path, t=title, s=score (rounded to 2dp), sn=snippet (omitted when empty). Cost note: `limit` controls returned hits; stage 2 fetches up to limit*2 bodies. Quoted phrases supported. Chain with wiki_fetch to read full bodies.",
  wiki_fetch:
    "Read full markdown for known paths (max 20 per call). Returns JSON: an array where success rows are {p,c,fm} (p=path, c=content, fm=frontmatter) and per-path failures are {p,err}. Partial-success: unknown paths return `err: 'path not in snapshot'` rather than failing the whole batch — inspect each row. Frontmatter is filtered by SENSITIVE_FRONTMATTER_KEYS. Use after wiki_search/wiki_list/citations surface a path.",
  wiki_list:
    "Enumerate discovered pages without fetching bodies. Optional filters: domain, type, tag, entity, concept (all case-insensitive). Pagination: `limit` (default 200, max 1000) and `offset`. Returns terse JSON {g,tot,off,lim,tr} where g is a domain→type→[{p,t}] grouping, tot=total matching rows before paging, off/lim/tr=offset/limit/truncated. Cheapest tool — use to scope before wiki_context.",
  wiki_upload:
